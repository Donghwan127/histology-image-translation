...
class Diffusion_FFPE(nn.Module):
    def __init__(self, pretrained_path=None, model_path="stabilityai/sd-turbo", lora_rank_unet=128, lora_rank_vae=4,
        ...
        # 2) Conch → projector
        conch_out_dim = 768     # vit_base output dim
        text_dim = 1024          # CLIP text embedding dim

        self.conch_proj = nn.Linear(conch_out_dim, text_dim)
    ...
    def forward(self, x, direction, text_emb):
        ...
        # ----------------------------------------
        # 2) ✅ CONCH feature + projection
        # ----------------------------------------
        with torch.no_grad():
            conch_feat = self.conch(x)    

        if isinstance(conch_feat, (list, tuple)):
            # vision_aided_loss.Conch 에서는 conv_multi_level일 때
            # x[2]가 (B, 768) CLS 임베딩이 되도록 이미 처리되어 있음.
            conch_feat = conch_feat[-1]         

        # (B) 4D 피처맵인 경우: GAP
        if conch_feat.dim() == 4:
            # (B, C, H, W) -> (B, C)
            conch_feat = conch_feat.mean(dim=[2, 3])

        # (C) 토큰 시퀀스인 경우: (B, L, 768) -> (B, 768)
        elif conch_feat.dim() == 3:
            # 1) CLS 토큰만 쓰려면:
            conch_feat = conch_feat[:, 0, :]

        conch_feat = self.conch_proj(conch_feat)     
        conch_feat = conch_feat.unsqueeze(1)         

        # ----------------------------------------
        # 3) ✅ concat:  [ original text | conch ]
        # ----------------------------------------
        text_combined = torch.cat([text_emb, conch_feat], dim=1)  # (B, 78, 1024)
